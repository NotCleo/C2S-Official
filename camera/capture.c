...
uint8_t spi_transfer(uint8_t data) {
    ...
    // Simple safety timeout (no infinite loops)
    int timeout = 0;
...
void i2c_init() {
    I2C_REG(0x40) = 0x0A; 
    usleep(1000);
    I2C_REG(0x100) = 0xC8; // Slow clock (200 divider)
}

// BLIND WRITE: Fire and Forget. No checking "Busy" bits.
void i2c_write_reg(uint8_t reg, uint8_t val) {
    I2C_REG(0x40) = 0x0A;
    usleep(200); 
    I2C_REG(0x100) = 0xC8; // Enforce slow speed

    I2C_REG(0x108) = 0x100 | (OV2640_ADDR << 1); 
    I2C_REG(0x108) = reg;                        
    I2C_REG(0x108) = 0x200 | val;                

    // Wait 10ms. This is an ETERNITY for I2C. 
    // It guarantees the transaction is finished.
    usleep(10000); 
}

// ==========================================
// 5. CONFIGURATION: YUV422 QVGA
// ==========================================
...
int main() {
    ...
    // Save EXACTLY the reported length. 
    // Python will handle padding if it's short.
